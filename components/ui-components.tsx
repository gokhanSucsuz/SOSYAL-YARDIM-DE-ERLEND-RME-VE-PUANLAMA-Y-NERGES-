...
export const CheckboxItem = ({ label, checked, onChange, isAlert = false, points = null }: any) => (
  <label className={`flex items-center min-h-[50px] p-3.5 border rounded-xl cursor-pointer transition-all active:scale-[0.99] touch-manipulation select-none ${
    ...
    <div className="flex-shrink-0 mr-3 flex items-center justify-center">
      <input
        type="checkbox"
        className={`w-5 h-5 rounded border-slate-300 transition-transform ${isAlert ? 'text-rose-600 focus:ring-rose-500' : 'text-teal-600 focus:ring-teal-500'}`}
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
      />
    </div>
    <div className="flex-1 flex justify-between items-center gap-2">
      <span className={`text-sm font-semibold leading-snug ${checked && isAlert ? 'text-rose-900 font-bold' : checked ? 'text-teal-950 font-bold' : 'text-slate-700'}`}>
        {label}
      </span>
      {points && (
        <span className={`text-xs font-black shrink-0 px-2 py-0.5 rounded-md ${checked ? (isAlert ? 'bg-rose-200 text-rose-900' : 'bg-teal-200 text-teal-900') : 'bg-slate-100 text-slate-500'}`}>
          +{points} P.
        </span>
      )}
    </div>
...
export const RadioItem = ({ label, name, checked, onChange, points }: any) => (
  <label className={`flex items-center min-h-[50px] p-3.5 border rounded-xl cursor-pointer transition-all active:scale-[0.99] touch-manipulation select-none ${
    checked ? 'bg-teal-50/60 border-teal-400 ring-1 ring-teal-400' : 'bg-white border-slate-200 hover:bg-slate-50'
  } print:border-slate-300 print:bg-white print:p-2`}>
    <div className="flex-shrink-0 mr-3 flex items-center justify-center">
      <input
        type="radio"
    ...
    <div className="flex-1 flex justify-between items-center gap-2">
      <span className={`text-sm font-semibold leading-snug ${checked ? 'text-teal-950 font-bold' : 'text-slate-700'}`}>
        {label}
      </span>
      {points !== undefined && (
        <span className={`text-xs font-black shrink-0 px-2 py-0.5 rounded-md ${checked ? 'bg-teal-200 text-teal-900' : 'bg-slate-100 text-slate-500'}`}>
          +{points} P.
        </span>
      )}
    </div>
